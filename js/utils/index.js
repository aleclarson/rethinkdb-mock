...
var arrayEquals, assertType, isArray, isArrayOrObject, isConstructor, isNullError, merge, objectEquals, pluckWithArray, pluckWithObject, resolveArray, resolveObject, sliceArray, typeNames, update, utils;
...
utils.expectArray = function(value) {
  var type;
  type = utils.typeOf(value);
  if (type !== "ARRAY") {
...
isNullError = function(m) {
  if (m === "Index out of bounds") {
    return true;
  }
  if (m.startsWith("No attribute")) {
    return true;
  }
  if (~m.indexOf("NULL")) {
    return true;
...
utils["default"] = function(self, value) {
  self._run = function() {
    var error, result;
    try {
      result = self._query._run();
    } catch (error1) {
      error = error1;
      if (!isNullError(error.message)) {
        throw error;
      }
    }
